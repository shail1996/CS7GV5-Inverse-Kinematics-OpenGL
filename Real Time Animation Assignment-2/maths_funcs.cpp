...
mat4 perspective(float fovy, float window_width, float window_height, float near, float far) {
	return perspective(fovy, window_width / window_height, near, far);
}
...
mat4 mat4_from_axis_rad(float radians, vec3 axis)
{
	versor v = quat_from_axis_rad(radians, axis.v[0], axis.v[1], axis.v[2]);
	return quat_to_mat4(v);
}

mat4 mat4_from_axis_deg(float degrees, vec3 axis)
{
	versor v = quat_from_axis_deg(degrees, axis.v[0], axis.v[1], axis.v[2]);
	return quat_to_mat4(v);
}
