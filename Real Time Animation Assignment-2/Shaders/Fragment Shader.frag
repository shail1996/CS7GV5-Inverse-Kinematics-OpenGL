...
const float material_kd = 0.8;  // specularStrength
const float material_ks = 0.8;
// fixed point light properties
vec3 light_position_world  = vec3 (0.0, 5.0, 25.0);
vec3 Ls = vec3 (1.0, 1.0, 1.0); // white specular colour
vec3 Ld = vec3 (0.7, 0.7, 0.7); // dull white diffuse light colour
vec3 La = vec3 (0.2, 0.2, 0.0); // grey ambient colour
  
// surface reflectance
vec3 Ks = vec3 (1.0, 1.0, 1.0); // fully reflect specular light
vec3 Kd = vec3 (0.30, 0.80, 0.10); // orange diffuse surface reflectance
vec3 Ka = vec3 (1.0, 1.0, 1.0); // fully reflect ambient light
float specular_exponent = 10.0 + specularChangeVal; // specular 'power'
...
// ambient intensity
	...
	vec3 Is = Ls * Ka * toon_shade_spec * material_ks ; // final specular intensity
